...
- **Role:** Binary Defense Artillery honeypot thread on TCP/8080 only (banning and threat feeds disabled) graded via UHBS generic TCP plugin.
...
Labs use Docker network `uhbs-lab` with `127.0.0.1` host binds only. Module F uses `source_root` pointing at the cloned upstream tree under `.local/labs/artillery`.
