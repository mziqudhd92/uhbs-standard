...
- **Role:** LLM-maintained async HTTP honeypot with extensive scanner trap families (fake VPNs, webshells, MCP, GraphQL, tarpits).
...
Labs use network `uhbs-lab`, host port `18094` → container `8080`. Telemetry directory: `.local/labs/flux-telemetry` (create before runs if Module C file gates are required).
