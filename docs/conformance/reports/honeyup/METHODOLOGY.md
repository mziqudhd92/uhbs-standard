...
- **Role:** Uploader honeypot mimicking weak `/uploads` PHP endpoints to collect malware and spray-and-pray upload attempts.
...
Labs use network `uhbs-lab`, host port `18091` → container `4000`. Telemetry directory: `.local/labs/honeyup-telemetry` (create before runs if Module C file gates are required).
